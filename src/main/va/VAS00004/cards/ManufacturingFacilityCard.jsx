  ...
  console.log("mf_rendered");
	...
	useEffect(() => {
	  ...
	  setArr({
		...
		AM_file,
		DE_file,
		contactDtls: obj?.contactDtls?.length ? obj.contactDtls : 
		[...arr.contactDtls, {
			contactPerson: "",
			contactMobileNo: "",
	...
	const update_MF = async ()=>{
		...
		console.log(dataObj, "xxxxxxxxxxxxx");
	...
	const addContact = (e)=>{
		...
		if(!list[list.length-1].contactPerson || !list[list.length-1].contactMobileNo){
			seterror({...error,
				contactDtls: "Please fill both detail first"})
		}
		else if(list[list.length-1].contactMobileNo?.length !== 10 ){
			seterror({...error,
				contactDtls: "Please fill 10 digit contact number"})
		}
		else {
			seterror({...error,
				contactDtls: ""})
	...
	const handle_contactDtl = (e,index)=>{
					...
					<div className='row'>
						...
						<div className="col-md-4">
							<div className="from-each-div">
								<div className='mf-factloading'>
									<p className="from-label-p">{"Present Factory Loading (%)"}<spna className='text-danger fs-5'>*</spna>&nbsp;&nbsp;
						...
						<div className='col-md-8'>
							<div className="row">
												...
												</tr> */}
										{
											...
											arr?.contactDtls?.map((item, i)=>(
												<tr key={i}>
													...
													name="contactPerson"
													onChange={(e)=>handle_contactDtl(e,i)} />}</td>
													<td>{<input
													type="number" className="form-control"
													value={item?.contactMobileNo}
													name="contactMobileNo"
													onChange={(e)=>handle_contactDtl(e,i)} />}</td>
													<td style={{display:"flex"}}>
													{(arr.contactDtls.length>1)&&<button 
													onClick={(e)=>delContact(i)}><i className="fa-regular fa-trash-can"></i></button>}
													{(i===arr.contactDtls.length-1)&&
														<button onClick={addContact} ><i className="fa-solid fa-plus"></i></button>
													}</td>
												</tr>
											))
										}
								...
								{error.contactDtls && <p style={{color:"red"}}>{error.contactDtls}</p>}
							</div>
