...
const LocationwiseSanctionedPostDefinitionForm = ({ editMode, post, dispatch, mode, rowId, setData, data, onClose, row, rowData, edtVal, setEdtVal, addVal, index, queryInputObj, msg, setMsg, msgTyp, setMsgTyp, errExp, set_errExp, }) => {
    ...
    useEffect(() => {
        setChooseData({
            ...
            loc: (addVal.lvlTypCd === "05"||edtVal.lvlTypCd === "05") ?  true: false 
        })
        if (mode !== 1) {
            setFormData({
                actFlg: "A",
                lvlRefCd: edtVal ? edtVal.lvlRefCd:"",
                noSancPost: edtVal ? edtVal.noSancPost:"",
                postCd: edtVal? edtVal.postCd:"",
                postNm: edtVal? edtVal.postNm:"",
                rankCd:edtVal? edtVal.rankCd:"",
    ...
    useEffect(() => {
        ...
        const fetchPostLovData = async () => {
            ...
            await axios
                .post(process.env.REACT_APP_API_URL_PREFIX + "/HRF00002/getAllPostDefn", obj, { headers })
                .then((res) => {
                    console.log(res.data);
    ...
    const handleSubmit = async (e) => {
        ...
        const addObj = {
            ...
            mst: {
                ...
                noSancPost:parseInt(formData.noSancPost),
                postCd: formData.postCd,
                rankCd: parseInt(formData.rankCd),
                subDivCd: formData.subDivCd ? formData.subDivCd : formData.lvlRefCd
            }
        }
        ...
        if (mode === 1)
            await axios.post(process.env.REACT_APP_API_URL_PREFIX + '/HRF00002/saveAdd', addObj, { headers }).then(res => {
                console.log(res.data)
        ...
        if (mode === 2)
            await axios.post(process.env.REACT_APP_API_URL_PREFIX + '/HRF00002/saveEdit', editObj, { headers }).then(res => {
                console.log(res.data)
                if (!res?.data?.appMsgList?.errorStatus) {
                    //TRUE OPERATION
                    fetchData()
    ...
    const handleConfirmation = async () => {
        ...
        await axios.post(process.env.REACT_APP_API_URL_PREFIX + '/HRF00002/saveDelete', deleteObj, { headers }).then(res => {
            console.log(res.data)
            if (!res?.data?.appMsgList?.errorStatus) {
                fetchData()
    ...
    return (
        <div>
            <div className="container">
                ...
                <h4 className="card-title">Location wise Sanctioned Post Definition {getFormTitle(mode)}</h4>
...
export default LocationwiseSanctionedPostDefinitionForm;