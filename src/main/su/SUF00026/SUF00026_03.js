...
const SecretQuestionSettingForm = () => {
    ...
    useEffect(() => {
        ...
        const fetchOpenData = async () => {
            ...
            await axios.post(process.env.REACT_APP_API_URL_PREFIX + '/SUF00026/openAddForm', obj, { headers }).then((res) => {
    ...
    const addtableRow = () => {
        const hasBlankFields = tableRow.some((row) => !row.qustDesc || !row.actFlg);
        ...
        if (hasBlankFields) {
            ...
            const updatedRows = tableRow.map((row) => ({
                ...row,
                errors: {
                    qustDesc: !row.qustDesc ? "Please fill in this field." : '',
    ...
    const finalSubmit = async (e) => {
        ...
        let body = {
            ...
            mst: tableRow.map((row) => ({
                actFlg: row.actFlg,
                qustDesc: row.qustDesc,
            })),
        }

        await axios.post(process.env.REACT_APP_API_URL_PREFIX + '/SUF00026/saveAdd', body, { headers }).then((res) => {
    ...
    return (
        <div openData={openData}>
            <div className="page-header">
                <div>
                    <h1 className="page-title">Secret Question Setting - Multiple Add</h1>
                ...
                <div className="ms-auto pageheader-btn">
                    ...
                    <Link
                        ...
                        to={`${process.env.PUBLIC_URL}/SUF00026_01`}
                    >
                        <span>
                            <i className="fe fe-log-in" />
                            &nbsp;
                        </span>
                        Secret Question Setting List
                    </Link>
                                            ...
                                            {row.error && <div className="error-message">{row.error}</div>}
                                                            ...
                                                            <td>
                                                                <div className="col-md-12 input-group">
                                                                    ...
                                                                    />
                                                                    {charCount[index].isTyping && <span className="input-group-text">{row.qustDesc.length}/50</span>}
                                                                ...
                                                                {row.errors.qustDesc && (
                                                                    <div className="error-message text-red text-center">{row.errors.qustDesc}</div>
                                                                )}
                                                            </td>
                                                            <td>
                                                                <select
...
export default SecretQuestionSettingForm;