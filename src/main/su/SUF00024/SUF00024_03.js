...
import { modLovColumns, errCdLovColumns } from "./columns";
...
const ErrDeffForm = () => {
    ...
    useEffect(() => {
        ...
        const fetchOpenData = async () => {
            ...
            await axios.post(process.env.REACT_APP_API_URL_PREFIX + '/SUF00024/openAddForm', obj, { headers }).then((res) => {
    ...
    const addtableRow = () => {
        const hasBlankFields = tableRow.some((row) => !row.modNm || !row.modId || !row.errDesc || !row.errType || !row.actFlg);
        ...
        if (hasBlankFields) {
            ...
            const updatedRows = tableRow.map((row) => ({
                ...row,
                errors: {
                    modNm: !row.modNm ? "Please fill in this field." : '',
                    modId: !row.modNm ? "Please fill in this field." : '',
                    errDesc: !row.errDesc ? "Please fill in this field." : '',
                    errType: !row.errType ? "Please fill in this field." : '',
    ...
    const finalSubmit = async (e) => {
        ...
        await axios.post(process.env.REACT_APP_API_URL_PREFIX + '/SUF00024/saveAdd', body, { headers }).then((res) => {
    ...
    return (
        <div openData={openData}>
            <div className="page-header">
                <div>
                    <h1 className="page-title">Error Definition - Multiple Add</h1>
                ...
                <div className="ms-auto pageheader-btn">
                    ...
                    <Link
                        ...
                        to={`${process.env.PUBLIC_URL}/SUF00024_01`}
                    >
                        <span>
                            <i className="fe fe-log-in" />
                            &nbsp;
                        </span>
                        Error Definition List
                    </Link>
            ...
            <Row>
                                            ...
                                            <table className="table  dta-tabl" style={{ background: 'white' }}>
                                                <thead>
                                                    <tr>
                                                        ...
                                                        <th></th>
                                                        <th>Module Code<span className="text-red">*</span></th>
                                                        <th>Module Name<span className="text-red">*</span></th>
                                                    ...
                                                    {tableRow.map((row, index) => (
                                                        <tr key={index}>
                                                            ...
                                                            <td>
                                                                ...
                                                                />
                                                                {row.errors.modId && (
                                                                    <div className="error-message text-red text-center">{row.errors.modId}</div>
                                                                )}
                                                            </td>
                                                            <td>
                                                                {/* Error Deffintion */}
                                                                ...
                                                                />
                                                                {row.errors.modNm && (
                                                                    <div className="error-message text-red text-center">{row.errors.modNm}</div>
                                                                )}
              ...
              <span className="input-group-text">
                {formData?.errDesc?.length}/300
                                                                ...
                                                                {row?.errors?.errDesc && (
                                                                    <div className="error-message text-red text-center">{row?.errors?.errDesc}</div>
                                                                )}
                                                            </td>
                                                            <td>
                                                                <select
                                                                ...
                                                                >
                                                                    ...
                                                                    {openData?.ddErrType?.map((item) => (
                                                                        <option value={item.value}>{item.label}</option>
                                                                    ))}

                                                                </select>
                                                                {row.errors.errType && (
                                                                    <div className="error-message text-red text-center">{row.errors.errType}</div>
                                                                )}
                                                            </td>
                                                            <td>
                                                                <select
