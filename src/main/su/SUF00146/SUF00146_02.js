...
export const ChangeRequestTypeMasterForm = ({ editMode, post, dispatch, mode, rowId, setData, data, onClose, row, rowData, index, queryInputObj, msg, setMsg, msgTyp, setMsgTyp, addVal, setEdtVal, edtVal, parMsg, setParMsg, parMsgTyp, setParMsgTyp, errExp, set_errExp, parErrExp, set_parErrExp, }) => {
  ...
  const [formData, setFormData] = useState({
    ...
    chgReqTypDesc: rowData ? rowData.chgReqTypDesc : '',
  ...
  const handleSubmit = async (e) => {
    ...
    const addObj =
    {
      ...
      mst: [
        {
          chgReqTypDesc: formData?.chgReqTypDesc,
    ...
    if (mode === 1)
      await axios.post(process.env.REACT_APP_API_URL_PREFIX + '/SUF00146/saveAdd', addObj, { headers }).then(res => {
        console.log(res.data)
    ...
    if (mode === 2)
      await axios.post(process.env.REACT_APP_API_URL_PREFIX + '/SUF00146/saveEdit', editObj, { headers }).then(res => {
        console.log(res.data)
        if (!res?.data?.appMsgList?.errorStatus) {
          //TRUE OPERATION
          fetchData()
  ...
  const handleConfirmation = async () => {
    const deleteObj = {
      ...
      mst: {
        ...
        chngReqTypCd: formData?.chngReqTypCd
      ...
      axios
        .post(process.env.REACT_APP_API_URL_PREFIX + '/SUF00146/saveDelete', deleteObj, { headers })
        .then((res) => {
          console.log(res.data);
          if (!res?.data?.appMsgList?.errorStatus) {
            fetchData();
  ...
  return (
    <div>
      ...
      <div className="container">
        ...
        Change Request Type Master  {getFormTitle(mode)}
        </h4>
        ...
        <form className="form-horizontal" onSubmit={(e) => handleSubmit(e, mode, data, setData, onClose)}>
          ...
          <div className=" row mb-4">
            <label className="col-md-3 form-label">
              Request Description <span className="text-red">*</span>
            </label>
            <div className="col-md-9 input-group">
              ...
              />{charCount.chgReqTypDesc && <span className="input-group-text">{formData.chgReqTypDesc?.length}/50</span>}
