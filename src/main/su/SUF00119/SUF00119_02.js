...
const ApiMasterForm = ({ editMode, post, dispatch, mode, rowId, setData, data, onClose, row, rowData, index, queryInputObj, msg, setMsg, msgTyp, setMsgTyp, editFormData, setEditFormData, defaultData, setDefaultData, openPageData, setOpenPageData }) => {
const headers = { Authorization: 'Bearer ' + getApiToken() };
if(mode === 1) openPageData = null;
  const fetchData = async () => {
    await axios.post(process.env.REACT_APP_API_URL_PREFIX + '/SUF00119/getListPageData', queryInputObj, { headers }).then((res) => {
...
useEffect(() => {
  ...
  setApiReqStruc(editFormData?.apiReqStruc? {jsObject: reqStruc}: null)
  ...
  useEffect(() => {
    ...
    const fetchApiCatLovData = async () => {
      await axios
        .post(process.env.REACT_APP_API_URL_PREFIX + "/SUF00119/getAllCatMst", apiCatLovObj, { headers })
        .then((res) => {
  ...
  useEffect(() => {
    ...
    const fetchModuleLovData = async () => {
      await axios
        .post(process.env.REACT_APP_API_URL_PREFIX + "/SUF00119/getAllModMst", modLovObj, { headers })
        .then((res) => {
  ...
  useEffect(() => {
    ...
    const fetchFormLovData = async () => {
      await axios
        .post(process.env.REACT_APP_API_URL_PREFIX + "/SUF00119/getFormMstByModMst", formLovObj, { headers })
        .then((res) => {
 ...
 useEffect(() => {
   ...
   const fetchAppLovData = async () => {
     await axios
       .post(process.env.REACT_APP_API_URL_PREFIX + "/SUF00119/getAllAppInfo", appLovObj, { headers })
       .then((res) => {
 ...
 const handleRowClickApp = (rowData) => {
  ...
  const submitStruc = async (e) => {
   ...
   console.log("000000REQ",editFormData);
    if (mode === 1)
    axios.post(process.env.REACT_APP_API_URL_PREFIX + "/SUF00119/saveDataMst/01", addReqObj, { headers }).then((res) => {
    ...
    axios.post(process.env.REACT_APP_API_URL_PREFIX + "/SUF00119/editDataMst/01", editReqObj, { headers }).then((res) => {
      if (res.data) {
  ...
  const submitResStruc = async (e) => {
    ...
    if (mode === 1)
    axios.post(process.env.REACT_APP_API_URL_PREFIX + "/SUF00119/saveDataMst/02", addResObj, { headers }).then((res) => {
    ...
    axios.post(process.env.REACT_APP_API_URL_PREFIX + "/SUF00119/editDataMst/02", editResObj, { headers }).then((res) => {
      if (res.data) {
  ...
  const apiDetailSubmit = async (e) => {
    ...
    if (mode === 1)
      await axios.post(process.env.REACT_APP_API_URL_PREFIX + '/SUF00119/saveDataMst/00', addObj, { headers }).then(res => {
    ...
    if (mode === 2)
      await axios.post(process.env.REACT_APP_API_URL_PREFIX + '/SUF00119/editDataMst/00', editObj, { headers }).then(res => {
        if (!res?.data?.appMsgList?.errorStatus) {
          //TRUE OPERATION
          fetchData()
    ...
    if (mode === 3)
      if (window.confirm("Are you sure? The record will be deleted parmanantly"))
        await axios.post(process.env.REACT_APP_API_URL_PREFIX + '/SUF00119/saveDelete', deleteObj, { headers }).then(res => {
          if (!res?.data?.appMsgList?.errorStatus) {
            fetchData()
