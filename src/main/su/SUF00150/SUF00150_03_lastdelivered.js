...
export default function Register() {
 ...
 const [showAlert, setShowAlert] = useState(false);
  ...
  const extUserTypCd = useFormInput('');
    ...
    mst:{
      ...
      setError(res?.data?.appMsgList?.list[0]?.errDesc + " (" + res?.data?.appMsgList?.list[0]?.errCd + ")");
        ...
        if(MobileOTP?.checked==='N' && EmailOTP?.checked==='N'){
          setMsgCountdownTimer(false);
          setMsg('You have to Choose an option to contionue. reload page and start again')

        }
        else{
        msgCountdownTimer === false ? setMsg('Previous Otp got Expired. please enter Resend OTP to continue'): setMsg("Enter valid OTP. OTP Will Expire in")}
				setCrossIcon(true)
			}
		}).catch((err)=>{
			console.log(err);
  ...
  otpFlag ? (
    ...
    {showRegisterButton ?(
      ...
      >
        ...
        >
          Resend OTP 
          in <span>{minutes < 10 ? "0" + minutes : minutes}:{seconds < 10 ? "0" + seconds : seconds}</span>
        </button>}
