...
export const UserTypeMasterForm = ({ editMode, post, dispatch, mode, rowId, setData, data, onClose, row, rowData, index, queryInputObj, msg, setMsg, msgTyp, setMsgTyp, addVal, setEdtVal, edtVal, parMsg, setParMsg, parMsgTyp, setParMsgTyp, errExp, set_errExp, parErrExp, set_parErrExp, }) => {
  ...
  const [formData, setFormData] = useState({
    userTypDesc: rowData ? rowData.userTypDesc : '',
    userTypCd: rowData ? rowData.userTypCd : '',
  ...
  const handleSubmit = async (e) => {
    ...
    const addObj =
    {
      ...
      mst: [
        {
          userTypDesc: formData.userTypDesc,
    ...
    if (mode === 1)
      await axios.post(process.env.REACT_APP_API_URL_PREFIX + '/SUF00148/saveAdd', addObj, { headers }).then(res => {
        console.log(res.data)
    ...
    if (mode === 2)
      await axios.post(process.env.REACT_APP_API_URL_PREFIX + '/SUF00148/saveEdit', editObj, { headers }).then(res => {
        console.log(res.data)
        if (!res?.data?.appMsgList?.errorStatus) {
          //TRUE OPERATION
          fetchData()
  ...
  const handleConfirmation = async () => {
    ...
    axios
      .post(process.env.REACT_APP_API_URL_PREFIX + '/SUF00148/saveDelete', deleteObj, { headers })
      .then((res) => {
        console.log(res.data);
        if (!res?.data?.appMsgList?.errorStatus) {
          fetchData();
    ...
    <div>
      <div className="container">
        ...
        <h4 className="card-title">
          User Type Description  {getFormTitle(mode)}
        </h4>
        ...
        <form className="form-horizontal" onSubmit={(e) => handleSubmit(e, mode, data, setData, onClose)}>
          ...
          <div className=" row mb-4">
            <label className="col-md-3 form-label">
              User Type Desc <span className="text-red">*</span>
            </label>
            <div className="col-md-9 input-group">
              <input
                ...
                placeholder="User Type Desscription"
                required
                maxLength={50}
                disabled={mode === 3 || mode === 4}

              />{charCount.userTypDesc && <span className="input-group-text">{formData.userTypDesc.length}/50</span>}
