...
import {ImageCategoryFormAdd} from "./WAF00001_02";
...
const ImageCategory = () => {
  ...
  useEffect(() => {
    const fetchOpenData = async () => {
      ...
      await axios
        .post(
          process.env.REACT_APP_API_URL_PREFIX + "/WAF00001/openForm",
          obj,
          { headers }
        )
        .then((res) => {
  ...
  useEffect(() => {
    ...
    const fetchModLovData = async () => {
      await axios
        .post(
          process.env.REACT_APP_API_URL_PREFIX + "/WAF00001/getAllPortal",
          modLovObj,
          { headers }
        )
        .then((res) => {
    ...
    selectRowModLov && fetchModLovData();
  }, [selectRowModLov]);

  const getPortalTitle = (obj) => {
    return modLovData[Number(Object.keys(obj)[0])]?.portalTitle;
  };

  const getportalId = (obj) => {
    return modLovData[Number(Object.keys(obj)[0])]?.portalId;
  };
  ...
  const handleRowClickModLov = (rowData) => {
    ...
    //         catNm: getcatNm(selectRowModLov)
  ...
  useEffect(() => {
    ...
    if (selectRowModLov) {
      setQueryInputObj({
        apiId: "WAA00004",
        criteria: {
          portalId: getportalId(selectRowModLov),
        },
      });
    }
  }, [selectRowModLov]);
    ...
    <>
      <div>
        <div className="page-header">
          <div>
            <h1 className="page-title">Image Category</h1>
                ...
                <div className="col-md-6">
                  <div className="input-group">
                    ...
                    <input
                      ...
                      value={
                        getPortalTitle(selectRowModLov)
                          ? getPortalTitle(selectRowModLov)
                          : ""
                      }
...
export const CreateNewAccountModal = ({
  ...
  useEffect(() => {
    ...
    if (mode === 1) {
      url = process.env.REACT_APP_API_URL_PREFIX + "/WAF00001/openAddForm";
    }
    if (mode === 2) {
      url = process.env.REACT_APP_API_URL_PREFIX + "/WAF00001/openEditForm";
      body = {
        apiId: "WAA00006",
        mst: {
          catId: rowData.id,
    ...
    if (mode === 3) {
      url = process.env.REACT_APP_API_URL_PREFIX + "/WAF00001/openDeleteForm";
      body = {
        apiId: "WAA00005",
        mst: {
          catId: rowData.id,
    ...
    if (mode === 4) {
      url = process.env.REACT_APP_API_URL_PREFIX + "/WAF00001/openViewForm";
      body = {
        apiId: "WAA00007",
        mst: {
          catId: rowData.id,
