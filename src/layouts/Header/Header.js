...
export function Header({jacketInfo,allNews,currentModuleId, headerProfImgAdd, favLinkList=[], set_favLinkList}) {
...
//  const [allNews, set_allNews] = useState([])
 ...
 console.log(jacketInfo, "YYYYYYYYYYYYYY");
 ...
 const locFlg = getScplAdContext()?.locFlg ? getScplAdContext()?.locFlg: "";
  ...
  const dummyUserFlg = getScplAdContext().dummyUserFlg;
      ...
      <Container fluid className="main-container">
        <div className="d-flex align-items-center">
          ...
          <div className="main-header-center ms-3 d-none d-lg-block" style={{marginTop:"0px"}}>
            <h4>{jacketInfo?.title}</h4>
            <h6>{currentLvlRefNm}</h6>
...
<Dropdown className="dropdown d-md-flex message">
        ...
        >
          ...
          <div className="me-2">
      <div style={{backgroundColor: ""}} className="avatar avatar-md brround cover-image">
                          {item?.newsBlinkFlg === "Y" ? <img className='blink' src="../../../new.png" width={25} />:
                            <span style={{fontSize: "18px"}} >{(item?.newsTitle || item?.newsText)[0].toUpperCase()}</span>}
                        </div>
      </div>
                    ...
                    >
                      {headerProfImgAdd ? 
                        ...
                        <div className="avatar  profile-user brround cover-image mini-prof-text">
                          <span>{dummyUserFlg==="Y"?getScplAdContext().regNm[0].toUpperCase():getScplAdContext().userNm[0].toUpperCase()}</span>
                        </div>
                        </>
                      }
                    </Dropdown.Toggle>
                    ...
                    >
                      <div className="drop-heading">
                        <div className="text-center">
                         ...
                         {dummyUserFlg==="Y"? <> <h5 className="text-dark mb-0">{getScplAdContext().regNm}</h5>
                         <small className="text-muted">{getScplAdContext().sysRegNo}</small></> :<> <h5 className="text-dark mb-0">{getScplAdContext().userNm}</h5>
                          <small className="text-muted">{getScplAdContext().userId}</small></>}
                        </div>
                      </div>
                      <div className="dropdown-divider m-0"></div>
                      {dummyUserFlg==="Y" && <Dropdown.Item
                        href={`${process.env.PUBLIC_URL}/SUF00033_05`}
                      >
                     ...
                     { 
                      getScplAdContext()?.dummyUserFlg === "N" &&
                     <>{!getScplAdContext()?.detailData?.forceToChangePwd &&<Dropdown.Item
                        onClick={(e)=>{e.preventDefault(); navigate("/SUF00033_03/")}}
                      >
